...
function Orion:CreateOrion(orionName)
    ...
    CloseBtn.MouseButton1Click:Connect(function()
        ...
        if isClosed then
            ...
            tween(MainFrame, {Size = UDim2.new(0, 60, 0, 66)}, 0.3):Play()
            tween(ContentFrame, {BackgroundTransparency = 1}, 0.2):Play()
            tween(TabFrame, {BackgroundTransparency = 1}, 0.2):Play()
            tween(ElementContainer, {BackgroundTransparency = 1}, 0.2):Play()
            for _, child in pairs(ContentFrame:GetChildren()) do
                if child ~= Header and child:IsA("GuiObject") then
                    tween(child, {BackgroundTransparency = 1}, 0.2):Play()
                end
            end
        else
            CloseBtn.Image = "rbxassetid://4988112250"
            ...
            tween(ContentFrame, {BackgroundTransparency = 0.3}, 0.2):Play()
            tween(TabFrame, {BackgroundTransparency = 0.2}, 0.2):Play()
            tween(ElementContainer, {BackgroundTransparency = 0}, 0.2):Play()
            for _, child in pairs(ContentFrame:GetChildren()) do
                if child ~= Header and child:IsA("GuiObject") then
                    local originalTrans = 0.3
                    if child:IsA("ScrollingFrame") then originalTrans = 0 end
                    tween(child, {BackgroundTransparency = originalTrans}, 0.2):Play()
                end
            end
        end
    end)
    ...
    UserInputService.InputBegan:Connect(function(input, gpe)
        if not gpe and input.KeyCode == hideKey then
            ...
            if isHidden then
                ...
                tween(MainFrame, {BackgroundTransparency = 1}, 0.2):Play()
                tween(MainStroke, {Transparency = 1}, 0.2):Play()
            else
                if isClosed then
                    tween(MainFrame, {Size = UDim2.new(0, 60, 0, 66)}, 0.3):Play()
                else
                ...
                tween(MainFrame, {BackgroundTransparency = 0.1}, 0.2):Play()
                tween(MainStroke, {Transparency = 0.7}, 0.2):Play()
            end
        end
    end)
    ...
    function SectionHandler:SetHideKey(newKey)
    ...
    function SectionHandler:GetHideKey()
        return hideKey
    end
